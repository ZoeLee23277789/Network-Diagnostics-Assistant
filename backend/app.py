...
from connectivity_tests import DEFAULT_CONFIG, load_latest_report, run_connectivity_suite
...
@app.get("/api/connectivity/config")
def get_connectivity_config() -> tuple:
    return jsonify(DEFAULT_CONFIG), 200


@app.post("/api/connectivity/run")
def run_connectivity_tests() -> tuple:
    config = request.get_json(silent=True) or {}
    report = run_connectivity_suite(config)
    status_code = 200 if report["summary"]["overall_status"] in {"pass", "warning"} else 207
    return jsonify(report), status_code


@app.get("/api/connectivity/latest")
def get_latest_connectivity_report() -> tuple:
    report = load_latest_report()
    if not report:
        return jsonify({"error": "No connectivity report has been generated yet."}), 404
